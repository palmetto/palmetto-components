...
<Canvas>
  <Story name="Theme Example">
    {() => {
      ...
      const [optionTileGroupValue, setOptionTileGroupValue] = useState(options[0].value);
      const customTheme = {
      ...
      return (
        ...
        >
          ...
          <OptionTileGroup
            ...
            value={optionTileGroupValue}
            onChange={event => setOptionTileGroupValue(event.target.value)}
            options={options}
            direction="row"
          />
          <Tabs
