...
Allows users to pick a value from a predefined list of options. Ideally, a SelectInput should be used when there are 5 or more options, otherwise consider using a radio group instead.
...
The `size` prop is also a `ResponsiveProp`, so it can be sized differently at each [breakpoint](?path=/story/design-tokens-breakpoints--page) by passing an object.
