...
Use the `isMulti` prop combined with passing one or more options in an array to the `value` prop to pre-selecte option(s).
...
Use the `autoFocus` prop to autofocus a SelectInput on render.
