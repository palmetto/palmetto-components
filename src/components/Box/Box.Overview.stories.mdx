...
<Canvas>
  <Story name="Flex Direction">
    ...
    <Box margin="lg 0 lg 0">
      <Box fontWeight="bold" margin="0 0 xs 0" as="p">
        IMPORTANT:
      </Box>
      when using the reverse CSS flex direction, the childGap ceases to work correctly,
      since it is meant to automatically add margin classes to all children elements
      except the last one, which in this case is reversed, via CSS only, the node
      order remains intact.
    </Box>
    <Box>
      <Box fontWeight="bold" as="span" margin="0 0 xs 0">
        INCORRECT
      </Box>{' '}
      usage with childGap prop
    </Box>
    <Box margin="lg 0 md 0" fontWeight="bold">
      Row Reverse
    </Box>
