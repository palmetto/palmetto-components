...
Set the `fontFamily` prop with a [font family](/?path=/story/design-tokens-font-family--page) token to set the size of the text in the Box.

<Canvas>
  <Story name="Font Family">
    {[...FONT_FAMILY_OPTIONS].map((fontFamily, i) => (
      <Box fontFamily={fontFamily} key={i} margin='0 0 md 0' fontSize="xl">
        {`${fontFamily}`}
        <Box fontSize="sm">
          {FONT_FAMILY_VALUES[fontFamily].value}
        </Box>
      </Box>
    ))}
  </Story>
