import Button from './Button';
import { action } from '@storybook/addon-actions';
import { Meta, Story, Canvas, ArgsTable } from '@storybook/addon-docs/blocks';

<Meta
  title="Components/Button"
  component={Button}
/>

# Button

Buttons are used as triggers for actions.
They are used in forms, toolbars, modal dialogs and as stand-alone action triggers.
Actions almost always occur on the same page.

<Canvas isExpanded>
  <Button onClick={() => alert('clicked')}>
    Button
  </Button>
</Canvas>

## Props

<ArgsTable of={Button} />

## Appearance

<Canvas>
  <Story name="Button">
     <Button>
      Button
    </Button>
  </Story>
</Canvas>

## Full Width

Use the `fullWidth` prop to allow the button to grow to its container's full width.

<Canvas>
  <Story name="Full Width">
    <Button fullWidth>
      Full Width
    </Button>
  </Story>
</Canvas>

## Size

Set the size of the button

<Canvas>
  <Story name="Size">
    <>
    <div style={{ marginBottom: '1rem'}}>
      <Button size="sm">
        Small
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button>
        Medium (default)
      </Button>
    </div>
    <div>
      <Button size="lg">
        Large
      </Button>
    </div>
    </>
  </Story>
</Canvas>

## Color Variants

You can render a button in a variety of colors based on the need.

<Canvas>
  <Story name="Colors">
    <>
    <div className="m-bottom-md p-md">
      <Button variant="primary">
        Primary
      </Button>
    </div>
    <div className="m-bottom-md p-md background-color-primary">
      <Button variant="primary" isInverse>
        Primary Inverse
      </Button>
    </div>
    <div className="m-bottom-md p-md background-color-primary-lightest">
      <Button variant="primary" isOutlined>
        Primary Outline
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="success">
        Success
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="danger">
        Danger
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="grey">
        Grey
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="light">
        Light
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="dark">
        Dark
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="white">
        White
      </Button>
    </div>
    <div style={{ marginBottom: '1rem'}}>
      <Button variant="black">
        Black
      </Button>
    </div>
    </>
  </Story>
</Canvas>

## Loading

Setting the `isLoading` prop to true will replace the button text with loading indicator and disable the button to prevent unintended submissions.
The width of the button will remain consistent to prevent content from shifting.

<Canvas>
  <Story name="Loading">
    <>
      <div style={{ marginBottom: '1rem' }}>
        <Button isLoading>
          Log  In
        </Button>
      </div>
      <div style={{ marginBottom: '1rem' }}>
        <Button isLoading>
          Button with a really long name. Width will not decrease when loading indicator is rendered.
        </Button>
      </div>
      <div style={{ marginBottom: '1rem' }}>
        <Button fullWidth isLoading>
          Full Width Loading Button
        </Button>
      </div>
    </>
  </Story>
</Canvas>

## Disabled

Use for actions that aren’t currently available, such as immediately after a form submission.
The interface should make it clear why the button is disabled and what needs to be done to enable it.

<Canvas>
  <Story name="Disabled">
    <>
    <div style={{ marginBottom: '1rem' }}>
      <Button isDisabled>
        Disabled Button
      </Button>
    </div>
    <div style={{ marginBottom: '1rem' }}>
      <Button fullWidth isDisabled>
        Disabled Full Width Button
      </Button>
    </div>
    </>
  </Story>
</Canvas>

## Event Callbacks

Callback functions can be passed to `onClick`, `onBlur`, and `onFocus` events.

<Canvas>
  <Story name="Event Callbacks">
    <Button
      onBlur={action('blur')}
      onClick={action('click')}
      onFocus={action('focus')}
    >
      click, focus, blur events
    </Button>
  </Story>
</Canvas>

## Custom Classes

Use the `className` prop to add a custom class, or classes to a Button.

<Canvas>
  <Story name="Custom Classes">
    <Button className="background-color-secondary">
      Custom Classes
    </Button>
  </Story>
</Canvas>
