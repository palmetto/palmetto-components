...
Use a toast to relay information to a user about the result of an action.
...
We've included sensible defaults for the duration of a toast before it dissapears, but if you require custom duration, follow these guidelines:
...
**DISMISSABLE TOASTS**
1. It is almost always a good idea to have a toast be dismissable. Since it
does not hinder the user experience, and yet provides a quick and easy
escape hatch, should a user want to clear it out of their view.
For this reason our toasts are all dismissable by default, but this can be overriden if need be.
...
### Dismissable (or lack thereof)

In cases where you'd like the toast to remain for a set amount of time 
without the user being able to clear it, you can use `isDismissable: false`.

<Canvas isExpanded>
  <Story name="Dismissable">
    {() => {
      const handleClick = () => {
        toast(`this toast can't be dismissed`, { isDismissable: false, duration: 10000 });
      };
      return (
        <>
          <Box childGap="md" direction="row">
            <Button onClick={handleClick}>
              Open non-dismissable toast.
            </Button>
          </Box>
        </>
      );
...
(In most cases, a multi-line toast is a sign that the message is too long,
but in rare cases where it is necessary...)
...
<Canvas isExpanded>
  <Story name="Multi-Line">
    {() => {
      const handleClick = () => {
        toast.success(
          'this toast has long text that goes on and on and on for a long while, until it breaks into multiple lines',
          { isDismissable: true, duration: 30000 }
        );
      };
      return (
        <>
      ...
      return (
        <>
          ...
          <Box childGap="md">
            ...
            <Button onClick={() => handleClick('jsx')} variant="light">
              custom JSX
            </Button>
          </Box>
        </>
