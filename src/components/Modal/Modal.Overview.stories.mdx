...
## With Custom Portal Target

<Canvas>
  <Story name="With Portal">
    {() => {
      const [showModal, setShowModal] = useState(false);
      const [showResponsiveModal, setShowResponsiveModal] = useState(false);
      const container = useRef(null);
      useEffect(() => {
        console.log('REF', container.current);
      }, []);
      return (
        <>
          <div className="m-bottom-md" ref={container}>
            <Button variant="light" onClick={() => setShowModal(true)}>
              Show Modal
            </Button>
            <Modal ariaLabelledBy="titleFooterBody" isOpen={showModal} onDismiss={() => setShowModal(false)} maxWidth="4xl" portalTarget={container.current}>
              <Modal.Header id="titleFooterBody" title="The Modal Title" onDismiss={() => setShowModal(false)} />
              <Modal.Body>Modal body content</Modal.Body>
              <Modal.Footer>This is content in the modal footer</Modal.Footer>
            </Modal>
...
<Canvas>
  ...
  {() => {
    const Wrapper = ({ children }) => {
      ...
      return (
        <>
          <Box ref={ref} className="m-bottom-md">
            {children({ ref })}
          </Box>
        </>
      )
        ...
        {({ ref }) => {
          ...
          return (
            <>
              ...
              <Modal ariaLabelledBy="titleFooterBody" isOpen={showModal} onDismiss={() => setShowModal(false)} maxWidth="4xl" portalTarget={ref.current}>
                <Modal.Header id="titleFooterBody" title="The Modal Title" onDismiss={() => setShowModal(false)} />
                <Modal.Body>Modal body content</Modal.Body>
                <Modal.Footer>This is content in the modal footer</Modal.Footer>
              </Modal>
