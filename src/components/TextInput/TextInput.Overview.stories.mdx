...
Set the size of the text input to `sm`, `md` or `lg`, but it can take these values in a `ResponsiveProp` object if it needs
 to be sized differently for each breakpoint.
