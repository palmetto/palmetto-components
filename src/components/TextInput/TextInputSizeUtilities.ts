...
import { TextInputProps, TextInputSize } from './TextInput';
 ...
 */
 export const computedResponsiveSize = (size: TextInputProps['size'], prop: 'containerPadding' | 'childPadding' | 'radius' | 'fontSize') => {
  if (size && !(typeof size === 'string') && typeof size === 'object') {
    return Object.entries(size).reduce((acc, [key, value]) => ({ ...acc, [key]: inputSizeMaps[prop][value] }), {});
  }

  return inputSizeMaps[prop][size ?? 'md'];
}