...
Much like a standard HTML table, our API relies on nesting certain key elements (abstracted as components) in order to build the table.
This allows for more flexibility within individual component APIs to be extended in the future without bloating a single component with props.

The components that we will use to craft a table are:
...
<Preview>
  <Story name="Loading">
    {() => {
      ...
      const tableData = [
        { id: 1, hello: 'random key', color: 'red', flavor: 'vanilla' },
        { id: 2, color: 'blue', flavor: 'chocolate' },
        { id: 3, color: 'green', flavor: 'strawberry' },
      ];
      return (
