    ...
    {() => {
      ...
      return (
        <OptionTile
          ...
          hover={{ borderColor: 'primary', shadow: 'lg', background: 'primary-lightest' }}
        >
          Hello world!
        </OptionTile>
      );
...
## Conditional styling for selected option

<Canvas isExpanded>
  <Story name="Selected Styling">
    {() => {
      const [isSelected, setIsSelected] = useState(false);
      return (
        <OptionTile
          ...
          name="hoverOptionTile"
          value="hoverOptionTile"
          background={isSelected ? 'primary-lightest': undefined}
        ...
        <Box direction={{ base: 'column', tablet: 'row' }} childGap="md">
          {options.map((option, index) => (
            <OptionTile
              ...
              name={option.name}
              value={option.value}
              background={option.value === selectedOption ? 'primary-lightest' : undefined}
              borderColor={option.value === selectedOption ? 'primary' : undefined}
            >
              <img src={option.img} style={{ objectFit: 'cover', width: '100%', height: '100px' }} />
              <p className="p-md">{option.label}</p>
            </OptionTile>
          ))}
        </Box>
      );
